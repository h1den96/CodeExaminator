...
function FloatingTimer({ durationMins, startedAt, availableUntil, strictDeadline, onTimeUp }: 
  ...
  return (
    <div style={{ 
      position: "fixed", top: "15px", right: "20px", zIndex: 10000,
      padding: "8px 14px", borderRadius: "20px", 
      background: isUrgent ? "rgba(254, 242, 242, 0.95)" : "rgba(255, 255, 255, 0.9)", 
...
export default function ExamRunner() {
      ...
      try {
        ...
        uniqueQuestions.forEach((q: any) => {
            if (q.student_mcq && q.student_mcq.length > 0) {
                initialAnswers[q.question_id] = q.allow_multiple ? q.student_mcq : q.student_mcq[0];
  ...
  const handleAnswerChange = (qId: number, value: any, type: string) => {
    ...
    if (type === 'mcq') payload.mcq_option_ids = Array.isArray(value) ? value : [value];
      ...
      {question.question_type === 'programming' ? (
        ...
        <div style={{ minHeight: "100vh", background: colors.bg, color: colors.text, padding: "80px 20px 20px 20px" }}>
            <div style={{ maxWidth: "800px", margin: "0 auto" }}>
                <div style={{ padding: "20px", background: colors.card, borderRadius: "12px", border: `1px solid ${colors.border}`, marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                    ...
                    {question.question_type === 'mcq' && (
                        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                            {question.options?.map(opt => {
                                ...
                                return (
                                    <label key={opt.option_id} style={{ display: "flex", alignItems: "center", gap: "15px", padding: "15px", border: `1px solid ${colors.border}`, borderRadius: "8px", cursor: "pointer", backgroundColor: isSelected ? "#eff6ff" : colors.bg }}>
                                        <input
                                            ...
                                            onChange={() => {
                                                if (question.allow_multiple) {
                                                    ...
                                                    const next = current.includes(opt.option_id) ? current.filter((id: number) => id !== opt.option_id) : [...current, opt.option_id];
                                                    handleAnswerChange(question.question_id, next, 'mcq');
                                                } else {
                                                    handleAnswerChange(question.question_id, opt.option_id, 'mcq');
                                                }
                                            }}
                                        />
                        ...
                        <div style={{ display: "flex", gap: "20px" }}>
                            {[true, false].map((val) => (
                                <button key={String(val)}
                                    ...
                                    style={{ flex: 1, padding: "20px", borderRadius: "8px", cursor: "pointer", fontWeight: "bold", border: answers[question.question_id] === val ? `2px solid #2563eb` : `1px solid ${colors.border}`, backgroundColor: answers[question.question_id] === val ? "#eff6ff" : colors.bg, color: answers[question.question_id] === val ? "#1d4ed8" : colors.text }}>
                                    {val ? "TRUE" : "FALSE"}
                                </button>
                            ))}
                        </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <button onClick={() => setCurrentQIndex(i => i - 1)} disabled={currentQIndex === 0} style={{ padding: "12px 24px", borderRadius: "8px", border: `1px solid ${colors.border}`, background: colors.card, cursor: "pointer", opacity: currentQIndex===0?0.5:1 }}>← Previous</button>
                    {currentQIndex < testData.questions.length - 1 ? (
                        <button onClick={() => setCurrentQIndex(i => i + 1)} style={{ padding: "12px 24px", borderRadius: "8px", border: "none", background: "#2563eb", color: "white", cursor: "pointer", fontWeight: "bold" }}>Next →</button>
                    ) : (
                        <button onClick={handleSubmit} style={{ padding: "12px 30px", borderRadius: "8px", border: "none", background: "#16a34a", color: "white", cursor: "pointer", fontWeight: "bold" }}>Submit Exam</button>
                    )}
                </div>
            </div>
        </div>
