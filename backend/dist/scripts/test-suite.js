...
const submissionService_1 = require("../../src/services/submissionService");
...
async function runPacedTest(totalRequests, batchSize) {
    ...
    for (let i = 0; i < totalRequests; i += batchSize) {
        ...
        console.log(`🚀 Dispatching Squad: Submissions ${i + 1} to ${i + currentBatchSize}...`);
        const squad = Array.from({ length: currentBatchSize }).map(async () => {
            try {
                // We reset the lock so the grader doesn't skip it
                await pool.query("UPDATE exam.submissions SET status = 'in_progress' WHERE submission_id = $1", [SUBMISSION_ID]);
                await submissionService_1.SubmissionService.submitAndGrade(SUBMISSION_ID, STUDENT_ID, pool);
                completed++;
            }
            catch (err) {
    ...
    console.log(`\n--- 📊 Paced Test Results ---`);
    console.log(`Success: ${completed}/${totalRequests} | Total Time: ${duration.toFixed(2)}s`);
}
