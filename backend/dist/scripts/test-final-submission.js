...
const submissionService_1 = require("../../src/services/submissionService"); // Adjust path as needed
...
async function runFinalTest() {
    ...
    try {
        ...
        await submissionService_1.SubmissionService.saveSingleAnswer(SUBMISSION_ID, STUDENT_ID, {
            question_id: QUESTION_ID,
            code_answer: studentCode, // Using your fixed variable here
        }, pool);
        // 2. NOW grade it
        const result = await submissionService_1.SubmissionService.submitAndGrade(SUBMISSION_ID, STUDENT_ID, pool);
        console.log("\n--- ✅ Submission Successful ---");
        console.log(`Final Grade: ${result.final_score.toFixed(2)}`);
        console.log(`Status: ${result.status}`);
        const dbCheck = await pool.query(`SELECT sa.question_grade, sa.eval_result 
       FROM exam.student_answers sa
       JOIN exam.submission_questions sq ON sa.submission_question_id = sq.submission_question_id
       WHERE sq.submission_id = $1 AND sq.question_id = $2`, [SUBMISSION_ID, QUESTION_ID]);
