...
async function runAudit() {
    ...
    for (const scenario of testScenarios) {
        ...
        try {
            ...
            const staticCheck = gradingService_1.GradingService.performStaticAnalysis(scenario.code, ["system(", "fork("], []);
            ...
            const result = await judge0Service_1.Judge0Service.runBatch(scenario.code, "cpp", scenario.testCases);
            const actualStatus = result.details[0].status.description;
            const actualGrade = result.grade;
            // 3. Αξιολόγηση Αποτελέσματος
            let success = true;
            if (scenario.shouldFail && actualStatus === "Accepted")
                success = false;
            if (scenario.minGrade && actualGrade < scenario.minGrade)
                success = false;
            if (success) {
