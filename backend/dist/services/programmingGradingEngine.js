...
exports.ProgrammingGradingEngine = void 0;
...
class ProgrammingGradingEngine {
    static safeDecode(str) {
        if (!str)
            return "";
        try {
    ...
    static async evaluate(input) {
        const { studentCode, testCases, points, category, signature, boilerplateCode, structuralRules = [], weightWb = 0.2, weightBb = 0.8, graceMode = "STANDARD", graceThreshold = 0.90, graceCap = 0.15, languageId = DEFAULT_LANG_ID } = input;
        // 1. Basic Static Security Scan
        const staticAnalysis = gradingService_1.GradingService.performStaticAnalysis(studentCode);
        ...
        const wbResult = await structuralAnalysisService_1.StructuralAnalysisService.analyze(studentCode, structuralRules);
        const S_wb = wbResult.score;
        if (!testCases || testCases.length === 0) {
            throw new Error("No test cases specified for verification.");
        }
        // 3. Prepare Runtime Harness
        const defaultHarness = boilerplateFactory_1.BoilerplateFactory.createFullHarness(category, signature, cleanedCode);
        let finalSourceCode = "";
        ...
        const batchResponse = await axios_1.default.post(`${RUNTIME_ENGINE_URL}/submissions/batch?base64_encoded=true&wait=false`, { submissions: payloads });
        ...
        while (attempts < 10 && !isDone) {
            const checkResponse = await axios_1.default.get(`${RUNTIME_ENGINE_URL}/submissions/batch?tokens=${tokens}&base64_encoded=true&fields=token,stdout,stderr,status,compile_output,memory`);
            results = checkResponse.data.submissions;
            if (results.every((r) => r.status && r.status.id > 2)) {
                isDone = true;
            }
            else {
                attempts++;
        ...
        const firstCompError = results.find((r) => r.status?.id === 6);
        let globalCompileLog = "";
        if (firstCompError) {
            try {
                const individualRes = await axios_1.default.get(`${RUNTIME_ENGINE_URL}/submissions/${firstCompError.token}?base64_encoded=true&fields=token,stdout,stderr,status,compile_output,message`);
        ...
        const astHealth = structuralAnalysisService_1.StructuralAnalysisService.calculateSyntacticHealth(studentCode);
        ...
        if (!isCompiled) {
            ...
            cleanDetails = results.map((r, idx) => ({
            ...
            cleanDetails = results.map((r, idx) => {
                ...
                const isPassed = gradingService_1.GradingService.smartCompare(actualOutput, expectedOutput);
...
exports.ProgrammingGradingEngine = ProgrammingGradingEngine;
