...
exports.ExamDiscoveryService = void 0;
class ExamDiscoveryService {
    static async getAvailableTestsForStudent(studentId, db) {
        const query = `
    SELECT 
      t.test_id, 
      t.title, 
  ...
  `;
    ...
    static async generateRandomTest(spec, db) {
        const { counts, config } = spec;
        const { topics } = config;
        const masterUniqueIds = new Set();
        const getUniqueBatch = async (type, limit, diff) => {
            if (limit <= 0)
                return 0;
    ...
    `, [finalIds]);
        return finalIds
            .map((id) => finalRes.rows.find((r) => r.question_id === id))
            .filter(Boolean);
    }
}
exports.ExamDiscoveryService = ExamDiscoveryService;
