...
class TestService {
    static async getTestWithQuestions(testId, db) {
    ...
    `, [testId]);
        ...
        test.questions = qRes.rows.map((q) => {
            if (q.question_type === "programming" && (!q.boiler_plate_code || q.boiler_plate_code.trim() === "")) {
                q.boiler_plate_code = boilerplateFactory_1.BoilerplateFactory.createFullHarness((q.category || "LINEAR"), q.function_signature || "");
            }
            return q;
        });
        for (const q of test.questions) {
            if (q.question_type === "mcq") {
                const optRes = await db.query(`SELECT option_id, option_text, option_id as id, option_text as text
           FROM exam.mcq_options 
           WHERE question_id = $1 
           ORDER BY option_id`, [q.question_id]);
                q.options = optRes.rows;
            }
        }
        return test;
    }
    static async reconstructTestFromSubmission(submissionId, db) {
