...
export const createProgrammingQuestion = async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
...
export const createMCQ = async (req: Request, res: Response) => {
  try {
    ...
    await AdminService.createMCQ({ title, body, difficulty, topic_ids, options, teacher_id });
...
export const createTF = async (req: Request, res: Response) => {
  try {
    ...
    if (typeof is_true !== 'boolean') return res.status(400).json({ error: "is_true must be boolean" });

    await AdminService.createTF({ title, body, difficulty, topic_ids, is_true, teacher_id });
