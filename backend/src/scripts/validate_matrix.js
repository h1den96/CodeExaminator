...
const jwt = require('jsonwebtoken'); // Added to dynamically mint fresh tokens
...
// Generate a dynamic fresh token valid for 2 hours
...
async function prepareTestSubmissionContext(client, questionIds) {
  ...
  // Generate dynamic JWT for student
  ...
  try {
    ...
    for (const q of questions) {
      ...
      try {
        ...
        // Strict evaluation: HTTP 200 AND full score (>= 9.9)
        ...
        if (isPassed) {
          ...
          // Print error trace whenever HTTP isn't 200 OR grade is 0
          if (true) {
            console.log(`\n--- Diagnostic Error Output for Question ${qId} (${q.title.trim()}) ---`);
