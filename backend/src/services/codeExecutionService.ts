...
export class CodeExecutionService {
    ...
    static async executeAndGrade(
        ...
        try {
            ...
            const structuralMaxPoints = maxPoints * 0.20; 
            ...
            if (checkRes.rowCount && checkRes.rowCount > 0) {
                const updateQuery = `
                    ...
                    RETURNING answer_id, question_grade
                `;
                await db.query(updateQuery, [studentCode, JSON.stringify(finalMeta), finalScore, submissionQuestionId]);
            } else {
                const insertQuery = `
                    INSERT INTO exam.student_answers 
                    (submission_question_id, code_answer, eval_result, question_grade, answered_at)
                    ...
                    RETURNING answer_id, question_grade
                `;
                await db.query(insertQuery, [submissionQuestionId, studentCode, JSON.stringify(finalMeta), finalScore]);
            }
