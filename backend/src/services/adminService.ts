...
export class AdminService {
    ...
    try {
      ...
      if (dto.question_type === "mcq" && dto.options) {
        for (const opt of dto.options) {
          ...
          const weight =
            typeof opt.score_weight === "number"
              ? opt.score_weight
              : opt.is_correct
                ? 1.0
                : 0.0;
    ...
    try {
      ...
      if (dto.slots && dto.slots.length > 0) {
        ...
        for (let i = 0; i < dto.slots.length; i++) {
          ...
          const dbType =
            s.question_type === "mcq" ? "multiple_choice" : s.question_type;
  ...
  static async updateProgrammingTestCases(
    questionId: number,
    testCases: any[],
  ) {
    const client = await examDb.connect();
    try {
      ...
      if (res.rowCount === 0) {
        throw new Error(
          "Programming question not found (or id is not a programming type)",
        );
      }
