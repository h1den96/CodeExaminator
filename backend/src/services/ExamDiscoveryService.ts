...
export class ExamDiscoveryService {
  ...
  static async generateRandomTest(spec: RandomizerSpec, db: Pool) {
    ...
    // Helper: Fetch a batch of questions for a specific type and difficulty
    const fetchBatch = async (
      type: 'mcq' | 'true_false' | 'programming',
      limit: number,
      diff: Difficulty | null // null = any difficulty
    ) => {
      if (limit <= 0) return [];
      ...
      // 1. Topic Filter (Check if question has ANY of the requested topics)
    ...
    // Strategy: If specific difficulty distribution exists (e.g., 3 Easy, 5 Hard), 
    // we fetch MCQs in buckets. For TF/Prog, we fetch generic (or you can apply the same logic).
    
    // --- 1. MCQ (Apply Difficulty Buckets) ---
    if (counts.mcq > 0) {
      if (difficulty_distribution) {
        if (difficulty_distribution.easy) {
          results.push(...await fetchBatch('mcq', difficulty_distribution.easy, 'easy'));
        }
        if (difficulty_distribution.medium) {
          results.push(...await fetchBatch('mcq', difficulty_distribution.medium, 'medium'));
        }
        if (difficulty_distribution.hard) {
          results.push(...await fetchBatch('mcq', difficulty_distribution.hard, 'hard'));
        }
      } else {
        // No distribution specified? Get random MCQs of any difficulty
        results.push(...await fetchBatch('mcq', counts.mcq, null));
      }
    }

    // --- 2. True/False (Generic for now) ---
    if (counts.tf > 0) {
      results.push(...await fetchBatch('true_false', counts.tf, null));
    }

    // --- 3. Programming (Generic for now) ---
    if (counts.prog > 0) {
